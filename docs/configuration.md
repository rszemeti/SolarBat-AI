...
```yaml
battery_soc: sensor.solis_battery_soc           # Battery state of charge (%)
battery_capacity: sensor.solis_battery_capacity # Battery capacity (kWh)
```

**Finding your entity IDs:**
1. Go to Developer Tools → States
2. Search for "solis" or "battery"
3. Copy the exact entity ID

**Note:** Entity names depend on your solax_modbus configuration. They may start with:
- `sensor.solis_...` (common)
- `sensor.solax_...` (alternative)
- `sensor.solis8_...` (if you named it solis8)

**Checking available inverter modes/slots:**

Before configuring, check which modes your inverter supports:

1. Go to Developer Tools → States
2. Find `select.solax_charger_use_mode` (or your inverter's mode entity)
3. Click on it to see available options
4. Note the EXACT names (they're case-sensitive!)

Common modes:
- "Self Use" (standard)
- "Grid First" (available on most)
- "Force Charge" (available on most)
- "Force Discharge" (NOT available on all inverters)

**Configuring mode names:**

```yaml
# Use the EXACT names from your inverter
mode_self_use: "Self Use"
mode_grid_first: "Grid First"
mode_force_charge: "Force Charge"
mode_force_discharge: "Force Discharge"  # Leave commented if not available
```

**If Force Discharge is not available:**

Comment out or omit `mode_force_discharge`. Pre-emptive discharge will use Self Use mode instead (drains via house consumption).
